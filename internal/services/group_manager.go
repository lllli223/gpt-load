...
func (gm *GroupManager) Initialize() error {
    loader := func() (map[string]*models.Group, error) {
        ...
        for _, group := range groups {
            ...
            // Parse param rename rules
            g.ParamRenameMap = make(map[string]string)
            if len(group.ParamRenames) > 0 {
                hasInvalidRules := false
                for key, value := range group.ParamRenames {
                    if valueStr, ok := value.(string); ok {
                        g.ParamRenameMap[key] = valueStr
                    } else {
                        logrus.WithFields(logrus.Fields{
                            "group_name": g.Name,
                            "rule_key":   key,
                            "value_type": fmt.Sprintf("%T", value),
                            "value":      value,
                        }).Error("Invalid param rename rule value type, skipping this rule")
                        hasInvalidRules = true
                    }
                }
                if hasInvalidRules {
                    logrus.WithField("group_name", g.Name).Warn("Group has invalid param rename rules, some rules were skipped. Please check the configuration.")
                }
            }
            ...
            logrus.WithFields(logrus.Fields{
                ...
                "param_rename_rules_count":    len(g.ParamRenameMap),

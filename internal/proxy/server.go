...
func (ps *ProxyServer) HandleProxy(c *gin.Context) {
    ...
    // Apply parameter renames first so downstream features see the normalized keys
    finalBodyBytes, err = ps.applyParamRenames(finalBodyBytes, group)
    if err != nil {
        response.Error(c, app_errors.NewAPIError(app_errors.ErrInternalServer, fmt.Sprintf("Failed to apply parameter renames: %v", err)))
        return
    }

    // Then apply value overrides
    ...
    // Detect streaming based on the final request body

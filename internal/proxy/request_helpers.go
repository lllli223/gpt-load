...
// applyParamRenames renames parameters in the JSON body if present according to group rules.
func (ps *ProxyServer) applyParamRenames(bodyBytes []byte, group *models.Group) ([]byte, error) {
    if group == nil || len(group.ParamRenameMap) == 0 || len(bodyBytes) == 0 {
        return bodyBytes, nil
    }

    var requestData map[string]any
    if err := json.Unmarshal(bodyBytes, &requestData); err != nil {
        logrus.Warnf("failed to unmarshal request body for param rename, passing through: %v", err)
        return bodyBytes, nil
    }

    for fromKey, toKey := range group.ParamRenameMap {
        if fromKey == toKey {
            continue
        }
        if val, ok := requestData[fromKey]; ok {
            requestData[toKey] = val
            delete(requestData, fromKey)
        }
    }
